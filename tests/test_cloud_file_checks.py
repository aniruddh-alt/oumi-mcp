...
class TestPreFlightIntegration:
    ...
    @patch("oumi_mcp_server.server.validate_datasets", return_value={})
    @patch("oumi_mcp_server.server.check_hardware", return_value=_MOCK_HARDWARE)
    @patch("oumi_mcp_server.server.check_cloud_readiness", return_value=_MOCK_CLOUD_READINESS)
    @patch("oumi_mcp_server.server.whoami", side_effect=Exception("no token"))
    def test_missing_file_mount_is_blocking(
        self, _hf, _cloud, _hw, _ds, tmp_path: Path
    ):
        from oumi_mcp_server.server import _pre_flight_check
